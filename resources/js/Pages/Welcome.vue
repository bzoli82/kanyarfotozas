...
const stats = computed(() => [
    { label: t('home.stats.photos'), value: nf.format(props.stats.photos ?? 0) },
    { label: t('home.stats.videos'), value: nf.format(props.stats.videos ?? 0) },
    { label: t('home.stats.locations'), value: nf.format(props.stats.locations ?? 0) },
    { label: t('home.stats.countries'), value: nf.format(props.stats.countries ?? 0) },
]);
</script>
    ...
    <PublicLayout transparent-header>
            ...
            <div class="relative h-[560px] w-full overflow-hidden sm:h-[620px] lg:h-[680px]">
                ...
                <template v-for="(slide, i) in heroSlides" :key="i">
                    ...
                    <img
                        ...
                        :class="i === heroIndex ? 'opacity-100 hero-slide--active' : 'opacity-0'"
                    />
                </template>
                ...
                <div class="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-4 pb-40 sm:px-6 lg:px-8">
                    ...
                    <p class="mt-4 max-w-md text-sm text-content/70 sm:text-base">
        ...
        <section class="mt-14 border-y border-border">
            <div class="mx-auto grid max-w-7xl grid-cols-2 divide-x divide-border border-x border-border sm:grid-cols-4 sm:border-x-0">
                <div v-for="f in features" :key="f.title" class="flex items-center gap-3 px-5 py-6">
        ...
        <section class="border-b border-border bg-surface-1">
            <div class="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
                ...
                <div class="mt-8 grid gap-6 sm:grid-cols-3">
                    <div v-for="(step, i) in steps" :key="step.title" class="flex flex-col gap-3">
        ...
        <section class="border-b border-border">
            <div class="mx-auto grid max-w-7xl grid-cols-2 gap-6 px-4 py-14 sm:px-6 lg:grid-cols-4 lg:px-8">
                <div v-for="s in stats" :key="s.label" class="text-center">
                    <div class="font-display text-3xl font-bold text-accent sm:text-4xl">{{ s.value }}</div>
